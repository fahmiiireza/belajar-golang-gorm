-- migrate -path user-service/db/migration -database "postgresql://postgres:postgres@localhost:5436/user-db?sslmode=disable" -verbose up 
...
INSERT INTO librarians (salary, employment_status, joining_date, user_id, created_by)
VALUES
    (40000, 'FULLTIME', '2023-01-01', (SELECT id FROM users WHERE username = 'librarian1'),(SELECT id FROM admins WHERE user_id = (SELECT id FROM users WHERE username = 'admin'))),
    (35000, 'PARTTIME', '2023-01-01', (SELECT id FROM users WHERE username = 'librarian2'),(SELECT id FROM admins WHERE user_id = (SELECT id FROM users WHERE username = 'admin')));
